...
const Shop = ({addItem}) => {
    ...
    return (
        <div className="shop">
            <div>
                <button onClick={() => setShow(true)}>Weapons</button>
                <button onClick={() => setShow(false)}>Potions</button>
            </div>

            {getShow && trader.weapons.map((x, i) => <WeaponItem addItem={addItem} key={i} item={x}/>)}
            {!getShow && trader.potions.map((x, i) => <WeaponItem addItem={addItem} key={i} item={x}/>)}
        </div>
    );
};
